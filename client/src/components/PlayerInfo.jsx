...
//checks health level & displays color accordingly
//data-for prop gives tooltip info over mana pt circle
const healthLevel = (points) => (points >= 12 ? 'bg-green-500' : points >= 6 ? 'bg-orange-500' : 'bg-red-500');
//creates blocks in health bar
//if there isn't much health then we'll have to decrease/increase the margin
//It calculates that
const marginIndexing = (index) => (index !== healthPoints - 1 ? 'mr-1' : 'mr-0');

const PlayerInfo = ({ player, playerIcon, mt }) => (
  //conditional margins
  //data tip allows us to know which img is this playr representing
  //spread the array of playr.health, take the keys of that property & map over that
  //map over items & get their index
  //health level calculates whether is it green, red or orange
